...
outputDir1 = [outputDir filesep 'runBLASTSpecific'];
if ~exist(outputDir1,'dir')
    mkdir(outputDir1)
end

closestSpecies = containers.Map;
...
geneNames = {};
blastDirs = dir('/mnt/extra/blast');
for i=1:length(blastDirs)
    if ~isempty(regexp(blastDirs(i).name,'RefSeq')) && blastDirs(i).isdir
        geneNamesFiles = dir(['/mnt/extra/blast' filesep blastDirs(i).name]);
        for j=1:length(geneNamesFiles)
            if ~strcmp(geneNamesFiles(j).name,'.') && ~strcmp(geneNamesFiles(j).name,'..')
                geneNames{end+1} = geneNamesFiles(j).name(1:regexp(geneNamesFiles(j).name,'.faa')-1);
            end
        end
    end
end
    ...
    for j=1:2
        ...
        for k=1:2
            ...
            for i=1:length(generaToSearch)
                speciesRef = closestSpecies(generaToSearch{i});
                referenceFaa = ['/mnt/extra/blast' filesep strrep(speciesRef,' ','_') 'RefSeq' filesep geneNames{z} '.faa'];
                if exist(referenceFaa,'file')
                    commandPrefix = ['blastp -db nr -query ' referenceFaa ' -gilist "' outputDir3 filesep];
                    command = [commandPrefix generaToSearch{i} '.txt" -outfmt "6 evalue aqcc sacc qstart qend sstart send" -out "' outputDir3 filesep generaToSearch{i} '.blast"'];
                    disp(command);
                    if actualRun
                        status=system(command);
                    end
